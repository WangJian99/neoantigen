...
### a perl script to get neoantigens from the file of mutation profiling
...
while (<CP>){
	...
	#my ($mt_pep,$wt_pep) = get_pep($nm,$cdot,$pdot,$aalen);
	...
	if (defined $mt_out && defined $wt_out){
		for my $alle (@{$hashHLA{$id}}){
			my $mt_mhc_out = run_netmhc($mt_out,$llen,$alle,"./NetMHC/${nm}_${pdot}_MT_${alle}");
			my $wt_mhc_out = run_netmhc($wt_out,$llen,$alle,"./NetMHC/${nm}_${pdot}_WT_${alle}");
			...
			for (my $i=0;$i<=$#ML;$i++){
				#print "$i\n";
				next if $ML[$i] =~ /^#/;
				if ($ML[$i] =~ /\s+HLA-[ABC]\*\d+\W\d+/ && $ML[$i] =~ /\s+[A-Z]{$llen}/){
					#print "ok\n";
					my @mline = split(/\s+/,$ML[$i]);
					my @wline = split(/\s+/,$WL[$i]);

					print NA "$id\t$gene\t$nm\t$cdot\t$pdot\t$vaf\t",join(";",@{$hashHLA{$id}}),"\t$alle\t$mline[3]\t$mline[13]\t$mline[14]\t$wline[3]\t$wline[13]\t$wline[14]\n";
				}	
			}
		}
		...
		for my $alle (@{$hashHLA{$id}}){
			...
			for (my $i=0;$i<=$#ML;$i++){
				next if $ML[$i] =~ /^#/;
				if ($ML[$i] =~ /\s+HLA-[ABC]\*\d+:\d+/ && $ML[$i] =~ /\s[A-Z]{$llen}/){
					my @mline = split(/\s+/,$ML[$i]);
					print NA "$id\t$gene\t$nm\t$cdot\t$pdot\t$vaf\t",join(";",@{$hashHLA{$id}}),"\t$alle\t$mline[3]\t$mline[13]\t$mline[14]\tNAN\tNAN\tNAN\n";
				}
			}
		}
	}else{
...
sub get_pep {
	...
	if ($pdot =~ /[pP]\.([A-Z])(\d+)([A-Z])$/){# substitution
		...
		my ($left,$right);
		if ($aa ne $wp ){
			print STDERR "Error: $acc $cdot $pdot have an error transcript.\n";
			return undef,undef;
		}else{
			if ($pos <= $len){
				$left = substr($prot,0,$pos-1);
		...
		$mtpep = $left . $mp . $right;
		$wtpep = $left . $aa . $right;	
	}elsif($pdot =~ /[pP]\.([A-Z])(\d+)([A-Z])fs\*\d+/){ # frame shift 
		my $wp = $1; 
		my $pos = $2; my $mp = $3;
		my $new_cds;
